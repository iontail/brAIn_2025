...
@misc.profiled_function
def window_partition(x, window_size): #feature map을 window로 쪼개기
    """
    Args:
        x: (B, H, W, C)
...
@misc.profiled_function
def window_reverse(windows, window_size, H, W): # window를 feature map 형태로 바꾸기
    """
    Args:
        windows: (num_windows*B, window_size, window_size, C)
        window_size (int): Window size
...
@persistence.persistent_class
class WindowAttention(nn.Module): # MCA layer in MAT
    ...
    def forward(self, x, mask_windows=None, mask=None):
        ...
        """
        B_, N, C = x.shape
        norm_x = F.normalize(x, p=2.0, dim=-1) # divede by max value -> -1 ~ 1
        q = self.q(norm_x).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) # (B_, H, N, c)
        k = self.k(norm_x).view(B_, -1, self.num_heads, C // self.num_heads).permute(0, 2, 3, 1) # (B_, H, c, N)
        v = self.v(x).view(B_, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) # Not Normlaized

        # Attention score
        attn = (q @ k) * self.scale # (B_, H, N, N)

        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B_ // nW, nW, self.num_heads, N, N) + mask.unsqueeze(1).unsqueeze(0) # (B, nW, H, N, N) + (1, nW, 1, N, N): 각 윈도우 마다의 마스크 값들을 더해준다 (mask는 -inf ~ 0)
            attn = attn.view(-1, self.num_heads, N, N) # (B, H, N, N)
 
        if mask_windows is not None: # 해당 마스크를 살릴 것인가 (unmask point가 하나라도 있으면 살리고, 아니면 0이 되버리게 함) -> 아예 없는 경우를 고려하는 코드라고 보면 됨
            ...
            with torch.no_grad():
                mask_windows = torch.clamp(torch.sum(mask_windows, dim=1, keepdim=True), 0, 1).repeat(1, N, 1) # 여기가 window에 하나라도 unmask 부분이 있으면 살리는 코드라고 생각하면 될 듯 -> 이후 레이어에 넘겨줌

        attn = self.softmax(attn) # mask 부분은 자연스럽게 0에 근사할 것임 (softmax(-100))
...
class SwinTransformerBlock(nn.Module):
    ...
    def calculate_mask(self, x_size):
        ...
        for h in h_slices:
            for w in w_slices:
                img_mask[:, h, w, :] = cnt # 각 구역에다가 번호 매기는 거라고 생각
                cnt += 1
        ...
        #  attn_mask 계산
        #    - mask_windows.unsqueeze(1): (nW, 1, ws*ws)
        #    - mask_windows.unsqueeze(2): (nW, ws*ws, 1)
        #    - 두 차이를 구하면, '같은 윈도우'인 경우 0, 
        #      '다른 윈도우'인 경우 양의 값(또는 음의 값)이 나옴.
        #    - masked_fill로 0이 아닌 경우 -100.0을 넣어
        #      서로 다른 윈도우끼리는 어텐션 점수가 낮아지게(-100.0) 만듦.
        # -----------------------------------------------------------
    ...
    def forward(self, x, x_size, mask=None):
        ...
        # cyclic shift
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2)) # h, w축으로 - shift_size 만큼 움직임
            if mask is not None:
        ...
        if self.shift_size > 0: #Shift한 경우에는 다시 원래 상태로 복구
        ...
        # FFN
        x = self.fuse(torch.cat([shortcut, x], dim=-1)) # concat한 다음에 그냥 fusion해버리네요... -> seq_len 안 늘어남
        x = self.mlp(x)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, x_size, mask=None):
        ...
        identity = x
        for blk in self.blocks: # 각 Stage 통과
...
class FirstStage(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        #-------------------------------
        # 메인 스테이지 통과 (Shifted window 포함됨 블록들로 구성된 단계)
        #-------------------------------
        x_size = x.size()[-2:]
        x = feature2token(x) # (B, L, C)
        ...
        for i, block in enumerate(self.tran):  # 64 to 16
            if i < mid: #donwsampling
                ...
                skips.append(x) # skip-connection for upsampling stage in main Stages
            elif i > mid:
                ...
                add_n = F.interpolate(add_n, size=x.size(1), mode='linear', align_corners=False).squeeze(1).unsqueeze(-1) # (B, L, 1)

                # 0.5 비율로 x는 유지 나머지는 style vector를 반영
                x = x * mul_map + add_n * (1 - mul_map)
                gs = self.to_style(self.down_conv(token2feature(x, x_size)).flatten(start_dim=1)) # to dim*2

                # 가장 응축된 단계(mid)로부터 global style vector를 찾음 (ws랑 global style(gs))
                style = torch.cat([gs, ws], dim=1)

        x = token2feature(x, x_size).contiguous() # (B, C, H, W)
        img = None
...
class SynthesisNet(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random', return_stg1=False):
        ...
        fea_16 = E_features[4] # mid stage의 feature
