...
class FirstStage(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        skips = []
        x, mask = self.conv_first(x, masks_in)  # input size #mask를 partial conv할 때 필요
        skips.append(x)
        ...
        for i, block in enumerate(self.tran):  # 64 to 16
            if i < mid:
                ...
                ws = self.ws_style(ws[:, -1]) # 가장 최근(마지막) style vector를 main steam에 집어넣기 위해 dim으로 projection
                add_n = self.to_square(ws).unsqueeze(1) # to 16*16
                add_n = F.interpolate(add_n, size=x.size(1), mode='linear', align_corners=False).squeeze(1).unsqueeze(-1) # (B, seq_len, 1)
                ...
                # 가장 응축된 단계(mid)로부터 style vector를 찾음 (ws랑 global style)
...
class SynthesisNet(nn.Module):
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random', return_stg1=False):
        ...
        # encoder
        x = images_in * masks_in + out_stg1 * (1 - masks_in) # 여기 값은 그냥 out_stg1이랑 똑같은 듯
