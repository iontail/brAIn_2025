...
class Deformable_Conv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size = 3, padding = 1, stride = 1, dilation = 2,
        ...
        nn.init_constant(self.p_conv.weight, 0)
        nn.init_constant(self.p_conv.bias, 0)
    ...
    def forward(self, x):
        ...
        # ????? x.size(1)이랑 x.size(2) 들어가야 하는 거 아닌가...???
        q_lt = torch.cat([
            torch.clamp(q_lt[..., :N], 0, x.size(1) - 1),
            torch.clamp(q_lt[..., N:], 0, x.size(2) - 1)
            ], dim = -1).long()

        q_rb = torch.cat([
            torch.clamp(q_rb[..., :N], 0, x.size(1) - 1),
        ...
        # 이미지 넘어가는 좌표 mask
        # 이것도 x.size(1)이랑 x.size(2)로 바꿔놨어요
        mask = torch.cat([
            p[..., :N].lt(self.pdding) + p[..., :N].gt(x.size(1) - 1 - self.padding),
            p[..., N:].lt(self.pdding) + p[..., N:].gt(x.size(2 ) - 1 - self.padding)
        ], dim = -1).type_as(p)
    ...
    def _get_x_q(self, x, q, N):
       ...
       index = index.contiguous().view(B, C, -1) # shape: (B, C, H * W * N)
    ...
    def _reshape_x_offset(x_offset, ks):
        ...
        x_offset = rearrange(x_offset, 'B C H W (KH KW) -> B C (H KH) (W HW)', KH = ks, KW = ks)
