...
class ToRGB(torch.nn.Module): # 1x1 kernel을 사용하는 ModulatedConv2d (+ Residual connection) 적용해서 RGB feature로 바꾸는 class
    ...
    def forward(self, x, style, skip=None):
        ...
        if skip is not None:
            # 코드 보니까 up이나 down 인자에 값을 주지 않는 거 보니까 크기는 안 변하는 듯
...
class Tokenization(nn.Module):
    def __init__(self,
        ...
        assert method in ['bn', 'ln'], f"We do not support {method} method."

        if deformable:
            self.proj = Deformable_Conv2d(in_channels, out_channels, kernel_size = kernel_size, padding = padding)
        else:
            self.proj = Conv2dLayer(in_channels, out_channels, kernel_size = kernel_size, padding = padding)
...
class MHA(nn.Module):
    def __init__(self, 
        ...
        head_dim = out_channels // head_num
        self.method = method
        ...
        self.conv_proj_mask = Tokenization(in_channels, out_channels, kernel_size = kernel_size,\
                                                  padding = padding_kv, stride = stride_kv,  deformable = deformable, method = method)
        ...
        self.softmanx = nn.SoftMax(dim = -1)
    ...
    def forward(self, x, mask = None):
        ...
        if mask is not None: # [mask] is 0 or -inf.
            # 여기가 문제네.... DCN한 것처럼 adaptive하게 mask 변경해야 하는데
            # 기존에는 input -> qkv proj -> attn
            # input -> dcn -> qkv proj with depthwise seperable conv. -> attn
            # 이렇게 바꿔서 dcn에 사용된 offset을 확인해서 mask update
            mask = self._calculate_mask(mask) # (B, 1, L)
    ...
    def _calculate_mask(self, mask): #DCN or CNN 통과한 x랑 마스크 맞춰주기 위해서 convolution 연산해서 mask update
        pass
...
class TransformerBlock(nn.Module):
    ...
    def forwad(self, x, x_size, low = None, mask = None):
        ...
        # FFN
        x = self.fuse(torch.cat([shortcut, x], dim = -1)) #concat with shortcut -> no residual connection -> learning low-frequency
        x = self.ffn(x)
...
class BasicLayer(nn.Module):
    def __init__(self,
        ...
        if frequency == 'high':
            ...
            self.conv = Conv2dLayerPartial(in_channels=dim * 2, out_channels=dim, kernel_size=3, activation='lrelu') # high-frequency도 고려
    ...
    def forward(self, x, x_size, mask=None, high = None):
        ...
        if high is not None:  # high-frequency가 추가로 들어오면
            if hasattr(self, 'dcn_fuse'): # high-frequenc
                high = token2feature(high)
...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512, use_noise=False, demodulate=True, activation='lrelu'):
        ...
        for i, depth in enumerate(depths):
            ...
            if ratios[i] < 1: # feature map이 작아지면
                ...
                merge_high = PatchUpsampling(dim, dim, down = ratios[i])
                merge_low = PatchUpsampling(dim, dim, down = ratios[i])
            else:
                merge = None

            self.high.append(
                BasicLayer(dim=dim, input_resolution=[res, res], depth=depth,
                           num_heads=num_heads, downsample=merge, frequency = 'high')
            )

            self.low.append(
                BasicLayer(dim=dim, input_resolution=[res, res], depth=depth,
                           num_heads=num_heads, downsample=merge, frequency = 'low')
            )


            self.early_fuse.append(
                Deformable_Conv2d(dim, dim)
            )
        ...
        self.to_style = F(in_features=dim, out_features=dim*2, activation=activation)
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        mid = len(self.tran) // 2
        for i, block in enumerate(zip(self.high, self.low, self.early_fuse)):  # 64 to 16 to 64
            if i < mid: #donwsampling
                if i == 0:
                    ...
                    high, _, high_mask = block[0](high, x_size, mask = mask)
                    low, x_size, low_mask = block[1](low, x_size, mask = mask, high = high)
                ...
                mul_map = torch.ones_like(high) * 0.5
                ...
                high = high * mul_map + add_n * (1 - mul_map)
                gs = self.to_style(self.down_conv(token2feature(high, x_size)).flatten(start_dim=1)) # to dim*2

                # 가장 응축된 단계(mid)로부터 global style vector를 찾음 (ws랑 global style(gs))
                style = torch.cat([gs, ws], dim=1)

        x = token2feature(high, x_size).contiguous() # (B, C, H, W)
        img = None
        ...
        for i, block in enumerate(self.dec_conv):
            x, img = block(high, img, style, skips[len(self.dec_conv)-i-1], noise_mode=noise_mode)
