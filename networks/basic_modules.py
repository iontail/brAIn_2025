...
class MHA(nn.Module):
    ...
    def _calculate_mask(self, mask, mask_size = None): #DCN or CNN 통과한 x랑 마스크 맞춰주기 위해서 convolution 연산해서 mask update
        mask = rearrange(mask, 'B 1 H W -> B 1 (H W)')
        return 
        ...
        for i, depth in enumerate(depths):
            ...
            self.high.append(
                BasicLayer(dim=dim, input_resolution=[res, res], depth=depth,
                           head_num = head_num, downsample=merge, frequency = 'high')
            )

            self.low.append(
                BasicLayer(dim=dim, input_resolution=[res, res], depth=depth,
                           head_num = head_num, downsample=merge, frequency = 'low')
            )
...
class SynthesisNet(nn.Module):
    def __init__(self,
        ...
        # first stage
        self.first_stage = FirstStage(img_channels, img_resolution=img_resolution, w_dim=w_dim, use_noise=False, demodulate=demodulate, use_4input = use_4input)
...
class Generator(nn.Module):
    ...
    def forward(self, images_in, masks_in, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False,
        ...
        if not return_stg1:
            img, high_freq = self.synthesis(images_in, masks_in, ws, noise_mode=noise_mode)
            return img, high_freq
        else:
            img, out_stg1, high_freq = self.synthesis(images_in, masks_in, ws, noise_mode=noise_mode, return_stg1=True)
            return img, out_stg1, high_freq
...
if __name__ == '__main__':
    ...
    batch = 1
    res = 512
    G = Generator(z_dim=512, c_dim=0, w_dim=512, img_resolution=512, img_channels=3, use_4input = False).to(device)
    ...
    with torch.no_grad():
        img, img_stg1, high_freq = G(img, mask, z, None, return_stg1=True)
    print('output of G:', img.shape, img_stg1.shape, high_freq)
