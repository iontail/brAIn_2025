...
class MHA(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        if mask is not None:
            mask, attn_mask = self._calculate_mask(mask) # (B, 1, L) # MCA: attn_mask = (-inf / 0)
            attn_mask = attn_mask.unsqueeze(-1).expand(-1, -1, -1, attn.size(-1)) # (B, 1, L, L) / mask 토큰에 해당하는 row에 대해서는 attention = 0
            attn = attn + attn_mask # (B, H, L, L) 

            attn_mask = attn_mask[..., 0].view(B, 1, H, W) # (B, 1, H, W)
    ...
    def _calculate_mask(self, mask, mask_size = None): #DCN or CNN 통과한 x랑 마스크 맞춰주기 위해서 convolution 연산해서 mask update
        if self.deformable:
            attn_mask = self.conv_proj_v._forward_mask(mask)

        else:
            attn_mask = F.conv2d(mask, self.proj_v.weight.detach())

        attn_mask = attn_mask.masked_fill(attn_mask == 0, 0).masked_fill(attn_mask != 0, 1)

        attn_mask = rearrange(attn_mask, 'B 1 H W -> B 1 (H W)')
...
if __name__ == '__main__':
    ...
    batch = 1
    res = 512
    G = Generator(z_dim=512, c_dim=0, w_dim=512, img_resolution=512, img_channels=3, use_4input = True).to(device)
